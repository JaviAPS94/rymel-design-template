 ...
 *
 * El contrato tiene que dar exactamente el mismo veredicto en el navegador
 * (project-admin, project-front) y en Node (project-back): el servidor valida
 * lo que recibe con este mismo código, y si el resultado dependiera del
 * entorno, validar en el servidor dejaría de significar nada.
 *
 * Esto falla el build si el código de producción:
 *   - declara dependencias de runtime que no sean las permitidas,
 *   - importa cualquier otro módulo que no sea relativo,
 *   - o toca una API propia de Node o del navegador.
 *
 * Los archivos `*.spec.ts` quedan fuera: no se publican y pueden importar el
 * runner de pruebas y los módulos de Node. Lo segundo es deliberado: la
 * prueba de ida y vuelta se ejecuta contra el volcado literal de las
 * plantillas reales, 370 KB de JSON que no tiene sentido convertir en un
 * módulo TypeScript solo para no leer un fichero.
 */

/**
 * La única dependencia de runtime admitida. El contrato valida fórmulas, y
 * las fórmulas las entiende el motor: duplicar aquí su tokenizador sería
 * volver a crear la divergencia que el motor compartido existe para evitar.
 */
const ALLOWED_DEPENDENCIES = ["@rymel/formula-engine"];
...
for (const file of listSourceFiles(SRC)) {
  ...
  for (const match of raw.matchAll(importPattern)) {
    ...
    if (isSpec && specifier.startsWith("node:")) continue;
    report(
      file,
...
if (runtimeDeps.length > 0) {
  violations.push(
    `package.json  declara dependencias de runtime no admitidas: ${runtimeDeps.join(", ")}`,
  );
}
